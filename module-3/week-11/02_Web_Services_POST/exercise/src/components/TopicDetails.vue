<template>
  <header class="flex">
    ...
    <div class="actions">
      <button
        class="btn-edit"
        v-on:click="
          $router.push({ name: 'EditTopicView', params: { topicId: topicId } })
        "
      >
        Edit
      </button>
  ...
  <div class="flex">
    ...
    <button
      class="btn-add"
      v-on:click="
        $router.push({ name: 'AddMessageView', params: { topicId: topic.id } })
      "
    >
      Add Message
    </button>
...
export default {
  ...
  methods: {
    deleteTopic() {
      if (
        confirm(
          "Are you sure you want to delete this topic and all associated messages? This action cannot be undone."
        )
      ) {
