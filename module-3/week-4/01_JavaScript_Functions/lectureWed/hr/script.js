...
const employees = [
  ...
  function sortEmployees(property) {
    ...
    employees.sort((lhs, rhs) => {
      if (typeof lhs[property] === "number") {
        return lhs[property] - rhs[property];
      } else {
        return lhs[property].localeCompare(rhs[property]);
      }
    });
 ...
 function onKeyPress(value) {
  ...
  function calculateAverageEmployment(employeeList) {
    ...
    const totalYears = employeeList.reduce((prevValue, employee) => prevValue + employee.years, 0);
