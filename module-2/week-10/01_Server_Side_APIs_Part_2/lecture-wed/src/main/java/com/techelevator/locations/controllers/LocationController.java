...
public class LocationController {
    ...
    @RequestMapping(path = "/locations/{locId}/tax", method = RequestMethod.GET)
    BigDecimal getTaxRate(@PathVariable int locId) {

        Location found = dao.getLocationById(locId);

        if (found != null) {
            BigDecimal rate = taxService.getTaxRate(found.getState());
            return rate;
        }
        else
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Location " + locId + " does not exist");
    }
}
