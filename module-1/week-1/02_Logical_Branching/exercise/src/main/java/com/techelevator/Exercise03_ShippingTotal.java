...
public class  Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        if (weightPounds <= 40){
            double newShippingRate = weightPounds * UP_TO_40_LB_RATE;
            return newShippingRate;
        } else if (weightPounds > 40) {
            double minimumShippingRate = MAX_WEIGHT_POUNDS * OVER_40_LB_RATE;
            double additionalShippingRate = OVER_40_LB_RATE * (weightPounds - 40);
            double  over40ShippingRate = additionalShippingRate + minimumShippingRate;
            return over40ShippingRate;
        }
        return 0.0;
    }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        if (weightPounds <= 40){
            double newShippingRate = weightPounds * UP_TO_40_LB_RATE;
            if (hasDiscount == true){
                double costWithDiscount = newShippingRate -(newShippingRate * .1);
            ...
            double  over40ShippingRate = additionalShippingRate + minimumShippingRate;
            if (hasDiscount == true){
                double costWithLargeDiscount = over40ShippingRate - (over40ShippingRate * .1);
                return costWithLargeDiscount;
            }return over40ShippingRate;
        }
        return 0.0;
