...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        if (numberOfNights <=2){
            double totalCost2Days = numberOfNights * DAILY_RATE;
            return totalCost2Days;
        } else if (numberOfNights >= 3) {
            double totalCost3Days = numberOfNights * DISCOUNT_RATE;
            return totalCost3Days;
        }return 0.00;
    ...
    public double calculateStayTotal(int numOfTotalNights, boolean includesParking) {
        if (numOfTotalNights <=2) {
            double totalCost2Days = numOfTotalNights * DAILY_RATE;
            if (includesParking == true) {
                double cost2DaysPlusPark = totalCost2Days + (PARKING_RATE * numOfTotalNights);
                return cost2DaysPlusPark;
            }
            return totalCost2Days;
        } else if (numOfTotalNights >= 3) {
                double totalCost3Days = numOfTotalNights * DISCOUNT_RATE;
                if (includesParking == true) {
                    double cost3DaysPlusPark = totalCost3Days + (PARKING_RATE * numOfTotalNights);
                    return cost3DaysPlusPark;
                }return totalCost3Days;
            } return 0.0;
    }
        ...
        if (numOfTotalNights <=2) {
            ...
            if (includesParking == true) {
                ...
                if (includesLateCheckout == true){
                    double cost2DaysPlusParkPlusLateCheck = cost2DaysPlusPark + LATE_CHECKOUT_FEE;
                    return cost2DaysPlusParkPlusLateCheck;
                }return cost2DaysPlusPark;
            } return totalCost2Days;
        } else if (numOfTotalNights >= 3) {
            double totalCost3Days = numOfTotalNights * DISCOUNT_RATE;
            if (includesParking == true) {
                double cost3DaysPlusPark = totalCost3Days + (PARKING_RATE * numOfTotalNights);
                if (includesLateCheckout == true){
                    double cost3DaysPlusParkPlusLateCheck = cost3DaysPlusPark +LATE_CHECKOUT_FEE;
                    return cost3DaysPlusParkPlusLateCheck;
                }return cost3DaysPlusPark;
            }return totalCost3Days;
        } return 0.0;
    }
