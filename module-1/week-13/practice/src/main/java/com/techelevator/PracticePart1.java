...
public class PracticePart1 {
	...
	*/
	public int Q04_carWashPrice(char typeOfWash, boolean isMorning, boolean isWeekend) {
		...
		if (typeOfWash == 'B' && isMorning == false && isWeekend == false) {
			return 8;
		}
		if (typeOfWash == 'B' && isMorning == true && isWeekend == false) {
			return 7;
		}
		if (typeOfWash == 'B' && isMorning == false && isWeekend == true) {
			return 10;
		}
		if (typeOfWash == 'B' && isMorning == true && isWeekend == true) {
			return 9;
		}
		if (typeOfWash == 'P' && isMorning == false && isWeekend == false) {
			return 10;
		}
		if (typeOfWash == 'P' && isMorning == true && isWeekend == false) {
			return 9;
		}
		if (typeOfWash == 'P' && isMorning == false && isWeekend == true) {
			return 12;
		}
		if (typeOfWash == 'P' && isMorning == true && isWeekend == true) {
			return 11;
		}
		if (typeOfWash == 'S' && isMorning == false && isWeekend == false) {
			return 12;
		}
		if (typeOfWash == 'S' && isMorning == true && isWeekend == false) {
			return 11;
		}
		if (typeOfWash == 'S' && isMorning == false && isWeekend == true) {
			return 14;
		}
		if (typeOfWash == 'S' && isMorning == true && isWeekend == true) {
			return 13;
		}
		return 0;
	}
	...
	*/
	public String Q06_stringBetweenTags(String stringToSearch, String tag) {
		// find the starting index of the first occurrence of the tag
		int startIndex = stringToSearch.indexOf(tag);
		if (startIndex == -1) {
			return ""; // tag not found
		}

		// adjust the starting index of the next occurrence of the tag
		startIndex += tag.length();

		// find the ending index of the next occurrence of the tag
		int endIndex = stringToSearch.indexOf(tag, startIndex);
		if (endIndex == -1) {
			return ""; // ending tag not found
		}
		// extract and return the substring between the tags
	...
	*/
	public int Q08_allowanceCalculator(int weekly, int gift, int bonus) {
		int allowance = weekly + (gift / (365 / 7)) + (bonus/ (4 * (365/7)));
		return allowance;
	}
	...
	*/
	public boolean Q09_isIncreasing(int[] numbers) {
		...
		for (int i = 0; i < numbers.length - 1; i++) {
			if (numbers[i] > numbers[i + 1]) { // had to change numbers[i++] to i+1
				result = false;
				break;
			}
		}
	...
	*/
	public Map<String, Integer> Q11_idealPets(String[] suggestedPets) {
		...
		Map<String, Integer> petCountMap = new HashMap<>();

		for (String pet : suggestedPets) {
			if (petCountMap.containsKey((pet))) {
				petCountMap.put(pet, petCountMap.get(pet) + 1);
			} else {
				petCountMap.put(pet, 1);
			}
		}
		return petCountMap;
